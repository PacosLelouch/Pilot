...
void AnimationPose::blend(const AnimationPose& pose)
{
    for (int i = 0; i < m_bone_poses.size(); i++)
    {
        ...
        if (sum_weight != 0)
        {
            ...
            bone_trans_one.m_position = Vector3::lerp(bone_trans_two.m_position, bone_trans_one.m_position, cur_weight); // cur_weight * bone_trans_one.m_position + (1.0f - cur_weight) * bone_trans_two.m_position;
            bone_trans_one.m_scale = Vector3::lerp(bone_trans_two.m_scale, bone_trans_one.m_scale, cur_weight); // cur_weight * bone_trans_one.m_scale + (1.0f - cur_weight) * bone_trans_two.m_scale;
            bone_trans_one.m_rotation = Quaternion::sLerp(cur_weight, bone_trans_two.m_rotation, bone_trans_one.m_rotation, false);
        }
    }
}
